...
import com.togethersports.tosproejct.response.RefreshApiResponse;
...
public class RefreshController {
    ...
    @PostMapping("/refresh")
    public ResponseEntity<RefreshApiResponse> validateRefreshToken(@RequestBody HashMap<String, String> bodyJson){
        ...
        if(map.get("status").equals("402")){
            ...
            RefreshApiResponse refreshApiResponseMessage = new RefreshApiResponse(Code.EXPIRED_TOKEN);
            return new ResponseEntity<>(refreshApiResponseMessage, HttpStatus.UNAUTHORIZED);
        }

        log.info("RefreshController - Refresh Token 이 유효.");
        RefreshApiResponse refreshApiResponseMessage = new RefreshApiResponse(Code.GOOD_REQUEST, map.get("accessToken"));
